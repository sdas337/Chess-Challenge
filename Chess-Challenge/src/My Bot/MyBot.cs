...
public class MyBot : IChessBot
{
    ...
    int searchBoard(int depth, int plyFromRoot, int alpha, int beta, bool nullMovePruningAllowed = true)
    {
        ...
        int bestScore = -900000, colorMoving = currentBoard.IsWhiteToMove ? 1 : 0, moveCount = 0;
        bool notRoot = plyFromRoot > 0, pvNode = beta - alpha > 1, inCheck = currentBoard.IsInCheck();
        ...
        /*var allMoves = currentBoard.GetLegalMoves(qsearch && !inCheck)?.OrderByDescending(move =>
        {
        ...
        for (int i = 0; i < allMoves.Length; i++)
        {
            ...
            if (currentMove == ttEntry.move)
                ...
                moveScores[i] -= 1000 * pieceValues[(int)currentMove.CapturePieceType] - (int)currentMove.MovePieceType;
            else
        ...
        //bool futile = depth <= 8 && (eval + 100 * depth) <= alpha;
        int reduction = 1;
        ...
        for (; moveCount < allMoves.Length; moveCount++)
        {
            ...
            bool importantMoves = pvNode || move.IsCapture || move.IsPromotion || inCheck;
...
#endif
            //LMR
            if(depth > 2 && !importantMoves && moveCount >= 9)
            {
                reduction = 2;
            } else
            {
            ...
            int Search(int newAlpha, int R = 1) => -searchBoard(depth - R, plyFromRoot + 1, -newAlpha, -alpha);

        Search:
            if (!fullSearch)
                moveScore = Search(alpha + 1, reduction);
            else
                moveScore = Search(beta, reduction);
